...
EPS = 0.05 # set EPS to be a range of values (0.01 to 4)
DTA = 0.1
A = 0
R = 10
...
def unbias_est(lda, rat, lklist, lcdlist, c, d, tnl):
    ...
    for k in range(0, R):
        tnl = tnl + (noiseG.sample(sample_shape = (1,)).numpy()[0])
...
for C in range(0, 10):
    for D in range(0, 10):
        ...
        if ratio != 0.0 and sum(KLDiv[C, D]) != 0.0:
            ...
            unbias_est(0.5, ratio, lHalfKList, lHalfCDList, C, D, 0)
            unbias_est(0.1, ratio, lTenthKList, lTenthCDList, C, D, 0)
            unbias_est(0.0875, ratio, lNextKList, lNextCDList, C, D, 0)
            unbias_est(0.0872, ratio, lHighKList, lHighCDList, C, D, 0)
            unbias_est(0.0871, ratio, lLowKList, lLowCDList, C, D, 0)
            unbias_est(0.08715, ratio, lDecideKList, lDecideCDList, C, D, 0)
            ...
            unbias_est(1.3992, ratio, lMinKList, lMinCDList, C, D, 0)
            unbias_est(0.4633, ratio, lMaxKList, lMaxCDList, C, D, 0)
...
estfile = open(f"fem_est_kld_end_gauss_noise_eps_{EPS}_mc.txt", "w", encoding = 'utf-8')
estfile.write("FEMNIST: Estimated KL Divergence In Order\n")
estfile.write(f"Gaussian Noise at End, Monte Carlo, Eps = {EPS}\n")
...
ratiofile = open(f"fem_ratio_kld_end_gauss_noise_eps_{EPS}_mc.txt", "w", encoding = 'utf-8')
ratiofile.write("FEMNIST: Ratio Between Exact KL Divergence And Estimator\n")
ratiofile.write(f"Gaussian Noise at End, Monte Carlo, Eps = {EPS}\n")
...
l0estfile = open(f"fem_l0est_kld_end_gauss_noise_eps_{EPS}_mc.txt", "w", encoding = 'utf-8')
l0estfile.write("FEMNIST: Unbiased Estimator Lambda Zero\n")
l0estfile.write(f"Gaussian Noise at End, Monte Carlo, Eps = {EPS}\n")
...
l1estfile = open(f"fem_l1est_kld_end_gauss_noise_eps_{EPS}_mc.txt", "w", encoding = 'utf-8')
l1estfile.write("FEMNIST: Unbiased Estimator Lambda One\n")
l1estfile.write(f"Gaussian Noise at End, Monte Carlo, Eps = {EPS}\n")
...
l2estfile = open(f"fem_l2est_kld_end_gauss_noise_eps_{EPS}_mc.txt", "w", encoding = 'utf-8')
l2estfile.write("FEMNIST: Unbiased Estimator Lambda Half\n")
l2estfile.write(f"Gaussian Noise at End, Monte Carlo, Eps = {EPS}\n")
...
l3estfile = open(f"fem_l3est_kld_end_gauss_noise_eps_{EPS}_mc.txt", "w", encoding = 'utf-8')
l3estfile.write("FEMNIST: Unbiased Estimator Lambda Tenth\n")
l3estfile.write(f"Gaussian Noise at End, Monte Carlo, Eps = {EPS}\n")
...
l4estfile = open(f"fem_l4est_kld_end_gauss_noise_eps_{EPS}_mc.txt", "w", encoding = 'utf-8')
l4estfile.write("FEMNIST: Unbiased Estimator Lambda Next\n")
l4estfile.write(f"Gaussian Noise at End, Monte Carlo, Eps = {EPS}\n")
...
l5estfile = open(f"fem_l5est_kld_end_gauss_noise_eps_{EPS}_mc.txt", "w", encoding = 'utf-8')
l5estfile.write("FEMNIST: Unbiased Estimator Lambda High\n")
l5estfile.write(f"Gaussian Noise at End, Monte Carlo, Eps = {EPS}\n")
...
l6estfile = open(f"fem_l6est_kld_end_gauss_noise_eps_{EPS}_mc.txt", "w", encoding = 'utf-8')
l6estfile.write("FEMNIST: Unbiased Estimator Lambda Low\n")
l6estfile.write(f"Gaussian Noise at End, Monte Carlo, Eps = {EPS}\n")
...
l7estfile = open(f"fem_l7est_kld_end_gauss_noise_eps_{EPS}mc.txt", "w", encoding = 'utf-8')
l7estfile.write("FEMNIST: Unbiased Estimator Lambda Decide\n")
l7estfile.write(f"Gaussian Noise at End, Monte Carlo, Eps = {EPS}\n")
...
l8estfile = open(f"fem_l8est_kld_end_gauss_noise_eps_{EPS}_mc.txt", "w", encoding = 'utf-8')
l8estfile.write("FEMNIST: Unbiased Estimator Lambda Minus\n")
l8estfile.write(f"Gaussian Noise at End, Monte Carlo, Eps = {EPS}\n")
...
l9estfile = open(f"fem_l9est_kld_end_gauss_noise_eps_{EPS}_mc.txt", "w", encoding = 'utf-8')
l9estfile.write("FEMNIST: Unbiased Estimator Lambda Triple\n")
l9estfile.write(f"Gaussian Noise at End, Monte Carlo, Eps = {EPS}\n")
...
l10estfile = open(f"fem_l10est_kld_end_gauss_noise_eps_{EPS}_mc.txt", "w", encoding = 'utf-8')
l10estfile.write("FEMNIST: Unbiased Estimator Lambda Min\n")
l10estfile.write(f"Gaussian Noise at End, Monte Carlo, Eps = {EPS}\n")
...
l11estfile = open(f"fem_l11est_kld_end_gauss_noise_eps_{EPS}_mc.txt", "w", encoding = 'utf-8')
l11estfile.write("FEMNIST: Unbiased Estimator Lambda Max\n")
l11estfile.write(f"Gaussian Noise at End, Monte Carlo, Eps = {EPS}\n")
...
l12estfile = open(f"fem_l12est_kld_end_gauss_noise_eps_{EPS}_mc.txt", "w", encoding = 'utf-8')
l12estfile.write("FEMNIST: Unbiased Estimator Lambda Pos\n")
l12estfile.write(f"Gaussian Noise at End, Monte Carlo, Eps = {EPS}\n")
...
l13estfile = open(f"fem_l13est_kld_end_gauss_noise_eps_{EPS}_mc.txt", "w", encoding = 'utf-8')
l13estfile.write("FEMNIST: Unbiased Estimator Lambda Neg\n")
l13estfile.write(f"Gaussian Noise at End, Monte Carlo, Eps = {EPS}\n")
