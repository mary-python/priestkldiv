...
for trial in range(7):
    ...
    for eps in epsset:
        ...
        if trial == 4 or trial == 5:
            statsfile.write(f"% Noise: {round((np.array(meanNoise) / meanValue[trial, EPS_FREQ])*100, 2)}\n\n")
        ...
        if trial == 4 or trial == 5:
            statsfile.write(f"% Noise: {round((np.array(minNoise) / minValue[trial, EPS_FREQ])*100, 2)}\n\n")
        ...
        if trial == 4 or trial == 5:
            statsfile.write(f"% Noise: {round((np.array(maxNoise) / maxValue[trial, EPS_FREQ])*100, 2)}\n\n")
