...
for trial in range(7):
    ...
    for T in Tset:
        ...
        if trial == 4 or trial == 5:
            statsfile.write(f"% Noise: {round((meanNoise.numpy() / meanValue[trial, T_FREQ])*100, 2)}\n\n")
        ...
        if trial == 4 or trial == 5:
            statsfile.write(f"% Noise: {round((minNoise.numpy() / minValue[trial, T_FREQ])*100, 2)}\n\n")
        ...
        if trial == 4 or trial == 5:
            statsfile.write(f"% Noise: {round((maxNoise.numpy() / maxValue[trial, T_FREQ])*100, 2)}\n\n")
